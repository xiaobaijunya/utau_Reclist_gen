...
if __name__ == "__main__":
    ...
    #把过短的采样行拼接起来，（length-1）/2=需要拼的采样行
    # data.CV_reclist)+len(data.VC_reclist) + len(data.CV_add_reclist
    min_length = length - 3
    rec_list = data.CV_reclist + data.VC_reclist
    print('开始拼接短录音条目：')
    for i in range(len(rec_list)):
        if len(rec_list[i]) <= min_length and len(rec_list[i]) != 0:
            for j in range(len(rec_list)):
                if len(rec_list[i]) + len(rec_list[j]) < length and len(rec_list[j]) != 0 and i != j:
                    print(rec_list[i], rec_list[j])
                    rec_list[i] = rec_list[i] + ['R'] + rec_list[j]
                    rec_list[j] = []  # 清空已使用的列表
                    print('new',rec_list[i])
                    break

    # 在拼接短行的循环结束后，清除rec_list中的空列表
    rec_list = [item for item in rec_list if item != []]  # 只保留非空列表

    with open('Reclist.txt', 'w',encoding='utf-8') as file:
        for reclist in rec_list:
    ...
    with open('Reclist.txt', 'r',encoding='utf-8') as file:
        ...
        # reclist1 = [rec.strip().split('_') for rec in reclist]
        # reclist1 = [[rec for rec in recs if rec != ''] for recs in reclist1]
    print(f'录音条目：\n总：{len(rec_list)+len(data.CV_add_reclist)} CVVC:{len(rec_list)} VR补充：{len(data.CV_add_reclist)}')
